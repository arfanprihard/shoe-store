  ...
  const fetchProducts = async () => {
    ...
    try {
      const [prodRes, brandRes, catRes] = await Promise.all([
        api.get('/admin/products', { params: { search, limit: 50 } }),
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                  ...
                  <th className="text-left py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">Harga</th>
                  <th className="text-left py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">Stok</th>
              ...
              <tbody>
                {products.map(p => (
                  <tr key={p.id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-gray-600 dark:text-gray-300">{p.brand?.name}</td>
                    <td className="py-3 px-4 font-medium text-gray-900 dark:text-white">Rp {p.price.toLocaleString('id-ID')}</td>
                    <td className="py-3 px-4">
                      <span className={`font-medium ${p.stock <= 5 ? 'text-red-500' : 'text-gray-700 dark:text-gray-300'}`}>{p.stock}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${p.isActive ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
